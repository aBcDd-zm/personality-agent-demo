...
当前线上版本已完成真实二维码邀请和完整海报保存：

- 公网首页为 `http://139.196.23.47/`。
- 二维码内容为首页入口 `/?ref=当前participantId`，用于邀请别人从头开始测评。
- 被邀请者扫码后进入首页，点击开始会生成新的 `participant_id`。
- 二维码不用于打开分享者结果页，项目不使用 `/share/{participant_id}` 旧分享结果页方案。
- 移动端顶部主海报会自动替换成完整 PNG 图片，用户可直接长按顶部海报保存。
- 当前完整 PNG 尺寸为 `1080 x 1880`，包含人格卡、人格名称、标签、大五人格雷达图、邀请文案和二维码。
- 该功能只改变前端保存/邀请体验，不改变 CSV 字段、数据库表结构或计分规则。
