...
当前已完成结果页体验：用户完成测评后会看到自己的结果页；结果页二维码是首页邀请入口 `/?ref=当前participantId`，用于邀请别人从头开始测评，不会打开分享者的结果页。结果页还提供“保存结果海报”按钮，会用原生 canvas 生成一张完整 PNG 海报，并显示为真实 `img#resultPosterImg`，手机端可长按保存整张海报。
...
- 别人扫码后进入首页重新测评，点击开始时会调用 `/api/session/start` 生成新的 `participant_id`。
- 项目不使用旧的 `/share/{participant_id}` 分享结果页方案，也不允许扫码查看分享者结果。
- “保存结果海报”按钮会手动绘制完整 PNG 海报，包含人格卡图片、人格名称、标签、说明/邀请文案和真实可扫码二维码。
- 手机端保存方式：点击“保存结果海报”，等待完整海报图片出现后，长按该图片并选择保存图片。
- 电脑端保存方式：点击生成后出现的“下载海报 PNG”链接。
- 导出的 PNG 中二维码已经绘制进 canvas，不是独立二维码 DOM 覆盖层，也不是只保存人格卡图片。
